                                ...
                                @forelse($packages as $package)
                                    <tr id="row-{{ $package->id }}">
                                        ...
                                        <td class="text-center">
                                            <span class="type-badge {{ $package->type === 'employer' ? 'type-employer' : 'type-seeker' }}">
                                                <i class="fas {{ $package->type === 'employer' ? 'fa-user-tie' : 'fa-user' }}"></i>
                                        ...
                                        <td class="text-center">
                                            <div class="action-buttons-group">
                                                <button onclick="viewPackage({{ $package->id }})" class="btn-action btn-action-view" title="View Details">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                                <button onclick="editItem({{ $package->id }})" class="btn-action btn-action-edit" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button onclick="toggleStatus({{ $package->id }})"
                                                    class="btn-action {{ $package->is_active ? 'btn-action-disable' : 'btn-action-enable' }}"
                                                    title="{{ $package->is_active ? 'Disable' : 'Enable' }}">
                                                    <i class="fas fa-{{ $package->is_active ? 'ban' : 'check-circle' }}"></i>
                                                </button>
                                                <button onclick="deleteItem({{ $package->id }}, '{{ addslashes($package->name) }}')"
                                    ...
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            <div class="empty-state">
                                                ...
                                                <p class="text-muted small">Click "Add Package" to create your first package.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
            ...
            <div class="modal-body">
                <form id="packageForm">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" id="itemName" class="form-control" placeholder="e.g. Pro Plan" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" id="itemPrice" class="form-control" min="0" placeholder="0" required>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label fw-semibold">Duration (Days) <span class="text-danger">*</span></label>
                                <input type="number" id="itemDuration" class="form-control" min="1" placeholder="30" required>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" id="itemJobLimit" class="form-control" min="0" placeholder="e.g. 10">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                <label class="form-label fw-semibold">Resume Views Limit (Seeker)</label>
                                <input type="number" id="itemResumeLimit" class="form-control" min="0" placeholder="e.g. 100">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <input type="color" id="itemBadgeColor" class="form-control form-control-color" value="#6c757d">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea id="itemDescription" class="form-control" rows="2" placeholder="Brief description of the package"></textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label class="form-label fw-semibold">Features (one per line)</label>
                        <textarea id="itemFeatures" class="form-control" rows="4" placeholder="Feature 1&#10;Feature 2&#10;Feature 3"></textarea>
                    </div>
                </form>
            </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-box me-2 text-primary"></i> Package Details
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<script>
    ...
    function viewPackage(id) {
        ...
        const body = document.getElementById('viewPackageBody');
        ...
        fetch(`/admin/packages/${id}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    body.innerHTML = `
                        <div class="row">
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-header bg-light">
                                        <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Package Info</h6>
                                    </div>
                                    <div class="card-body">
                                        <table class="table table-borderless table-sm mb-0">
                                            <tr>
                                                <td class="fw-bold">Name</td>
                                                <td><strong>${pkg.name}</strong> ${pkg.is_featured ? '<span class="featured-badge"><i class="fas fa-star"></i> Featured</span>' : ''}</td>
                                            </tr>
                                            <tr>
                                                <td class="fw-bold">Type</td>
                                                <td><span class="type-badge ${pkg.type === 'employer' ? 'type-employer' : 'type-seeker'}"><i class="fas ${pkg.type === 'employer' ? 'fa-user-tie' : 'fa-user'}"></i> ${pkg.type.charAt(0).toUpperCase() + pkg.type.slice(1)}</span></td>
                                            </tr>
                                            <tr>
                                                <td class="fw-bold">Price</td>
                                                <td><strong>PKR ${new Intl.NumberFormat().format(pkg.price)}</strong></td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>${pkg.is_active ? '<span class="status-badge status-completed"><i class="fas fa-check-circle"></i> Active</span>' : '<span class="status-badge status-failed"><i class="fas fa-times-circle"></i> Inactive</span>'}</td>
                                            </tr>
                                            ${pkg.display_order !== null ? `<tr><td class="fw-bold">Display Order</td><td>${pkg.display_order}</td></tr>` : ''}
                                        </table>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-header bg-light">
                                        <h6 class="mb-0"><i class="fas fa-list me-2"></i> Features</h6>
                                    </div>
                                    <div class="card-body">
                                        ${pkg.description ? `<p><strong>Description:</strong> ${pkg.description}</p><hr>` : ''}
                                        ${pkg.features && pkg.features.length > 0 ? `
                                            <ul class="list-unstyled">
                                                ${pkg.features.map(f => `<li><i class="fas fa-check-circle text-success me-2"></i> ${f}</li>`).join('')}
                                            </ul>
                                        ` : '<p class="text-muted">No features listed.</p>'}
                                        ${pkg.job_posts_limit ? `<p><strong>Job Posts Limit:</strong> ${pkg.job_posts_limit}</p>` : ''}
                                        ${pkg.resume_views_limit ? `<p><strong>Resume Views Limit:</strong> ${pkg.resume_views_limit}</p>` : ''}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    body.innerHTML = `<div class="alert alert-danger">Failed to load package details.</div>`;
                }
            })
            .catch(() => {
                body.innerHTML = `<div class="alert alert-danger">Failed to load package details.</div>`;
